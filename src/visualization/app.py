...
# 3️⃣ 然后显示四个指标卡片
high_risk_events = len(filtered_events[filtered_events['risk_level'] == '高'])
total_penalty = filtered_events['penalty_amount'].sum()
neg_ratio = len(filtered_events[filtered_events['sentiment'] == '负面']) / total_events if total_events > 0 else 0
...
with tab1:
    ...
    with col_right:
        high_cnt = len(filtered_events[filtered_events['risk_level'] == '高'])
        medium_cnt = len(filtered_events[filtered_events['risk_level'] == '中'])
        low_cnt = len(filtered_events[filtered_events['risk_level'] == '低'])
...
with tab3:
    ...
    high_risk_entities = dynamic_entity[dynamic_entity['risk_level'] == '高'].head(15)
