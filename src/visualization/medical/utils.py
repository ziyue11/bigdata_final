...
# 项目根目录：当前文件在 src/visualization/medical/，向上三级到达项目根
PROJECT_ROOT = Path(__file__).resolve().parents[3]
PROCESSED_DIR = PROJECT_ROOT / "data" / "medical" / "processed"
ANALYSIS_DIR = PROJECT_ROOT / "data" / "medical" / "analysis"


def load_all_data(industry: str = "medical") -> Dict[str, pd.DataFrame]:
    """加载医疗项目的所有数据文件"""
    data = {}

    risk_event = pd.read_csv(PROCESSED_DIR / "risk_event_table.csv", encoding="utf-8")
    ...
    summary = pd.read_csv(ANALYSIS_DIR / "summary_stats.csv", encoding="utf-8")
    risk_type = pd.read_csv(ANALYSIS_DIR / "risk_type_stats.csv", encoding="utf-8")
    time_trend = pd.read_csv(ANALYSIS_DIR / "risk_time_trend.csv", encoding="utf-8")
    region = pd.read_csv(ANALYSIS_DIR / "region_risk_stats.csv", encoding="utf-8")
    entity = pd.read_csv(ANALYSIS_DIR / "entity_risk_rank.csv", encoding="utf-8")
    sentiment = pd.read_csv(ANALYSIS_DIR / "sentiment_trend.csv", encoding="utf-8")
    relation = pd.read_csv(ANALYSIS_DIR / "risk_relation_matrix.csv", encoding="utf-8")
    warning = pd.read_csv(ANALYSIS_DIR / "risk_warning_cases.csv", encoding="utf-8")
...
def apply_filters(
    ...
    if search_keyword and search_keyword.strip():
        ...
        mask = (
            ...
            | filtered.get("violation_reason", pd.Series(dtype=str)).fillna("").astype(str).str.lower().str.contains(keyword)
        )
