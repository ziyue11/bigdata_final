...
def plot_entity_rank(df: pd.DataFrame, top_n: int = 10, sort_by: str = 'avg_risk_score') -> go.Figure:
    ...
    fig = px.bar(
        ...
        color='risk_level', color_discrete_map={'高': '#d62728', '中': '#ff7f0e', '低': '#2ca02c'},
        text=sort_by,
...
def plot_sentiment_trend(df_sentiment: pd.DataFrame, df_risk_time: pd.DataFrame) -> go.Figure:
...
def plot_risk_score_distribution(df: pd.DataFrame) -> go.Figure:
    ...
    fig = px.histogram(df, x='risk_score', nbins=30, color='risk_level',
                       color_discrete_map={'高': '#d62728', '中': '#ff7f0e', '低': '#2ca02c'},
                       title='风险评分分布 (0-100)',
